...
••••Make sure the file path is the same, and that it pulls data from lab3 and exports to lab4••••

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_knit$set(root.dir = "~/Downloads/UMich_Bio201_F19/")
```
...
scfa_data %>%
  group_by(semester) %>%
  summarize(mean_butyrate = mean(mm_per_kg_butyrate, na.rm = TRUE)) 
```
...
scfa_data %>%
  ...
  summarize(mean_butyrate = mean(mm_per_kg_butyrate))
```
...
```{r}
scfa_data %>%
  #filter(!is.na(mm_per_kg_butyrate)) %>% 
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(mm_per_kg_butyrate, na.rm = TRUE))
```
...
scfa_data %>%
 filter(!is.na(mm_per_kg_butyrate)) %>%
  group_by(semester, supplement_consumed) %>%
  summarize(mean_butyrate = mean(mm_per_kg_butyrate), 
            sd_butyrate = sd(mm_per_kg_butyrate), 
            n_butyrate = n())
```
