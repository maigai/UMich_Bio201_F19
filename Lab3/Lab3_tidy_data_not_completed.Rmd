...
knitr::opts_knit$set(root.dir = "~/Downloads/UMich_Bio201_F19/Lab3/")
```
...
scfa_indv_qc <- scfa_indv %>%
  scfa_indv <- rename_all(scfa_indv, tolower) %>% 
  mutate(scfa_indv, sample_kg = sample_wt_g / 1000) %>% 
  mutate(scfa_indv, ht_cm = ht_in * 2.54) %>% 
  mutate(scfa_indv, wt_kgs = wt_lbs / 2.205) %>% 
...
Impor the scfa_wkly dataset. Complete the following QC measures as one long series of commands using pipes:
