...
knitr::opts_knit$set(root.dir = "~/Downloads/UMich_Bio201_F19-master/Lab3/")
```
...
The abbreviated output below gives the first several columns and the first ten rows of the data frame. You’ll notice that at the bottom of the output, it tells us additional rows and columns. In addition, the output tells us the variable type of each column.
...
### Summary functions
The functions below can provide useful information about a data frame. Run each on the data frame `scfa_indv`. Briefly describe what each function tells you about the data frame. 
...
dim returns the dimensions of the data frame, rows by columns 
colnames returns the names of each column, ie sex, age, race, etc (research varaibles)
rownames returns the names of each row, user id in our case 
glimpse retuns a truncated version of the data as a preview 
str returns differently formatted version of glimpse
...
select(scfa_indv, supplement_consumed, use_data, )
...
* Rename columns as described in sections above (snake case, units) #done 
* Convert measurements from US Customary to metric units (1 kg = 2.205 pounds, 1 m = 35.274 inches ) #done 
* Round participant height and weight to 0 decimal #done
* Subset for samples within the weight limits #done
* Subset for samples that were frozen within 24 hours #done
* Convert sample weights to kilograms #already done
* Calculate mmol/kg for each SCFA #done 
* Calculate the total SCFA # done 
* Round all SCFA measurments to 2 decimal #done 
* Drop intermediate columns used for calculations #done
...
```{r}
  ...
  rename_all(tolower) %>%
  mutate(ht_cm = round(ht_in * 2.54),
         wt_kgs = round(wt_lbs / 2.205)
         ) %>%  
  filter(sample_wt_g > 0.10) %>% 
  filter(notes != "frozen >24hrs") %>% 
  mutate(sample_kg = sample_wt_g * 1000, 
         mm_per_kg_acetate =  acetate_mm / sample_kg,
         mm_per_kg_butyrate = butyrate_mm / sample_kg,
         mm_per_kg_propionate = propionate_mm / sample_kg, 
         total_scfa_mm = acetate_mm + butyrate_mm + propionate_mm, 
         acetate_mm = round(acetate_mm, digits = 2),
         butyrate_mm = round(butyrate_mm, digits = 2), 
         propionate_mm = round(propionate_mm, digits = 2)
         ) %>% 
  select(-ht_in, -wt_lbs, -sample_wt_g, -sample_kg)
...
Import the scfa_wkly dataset. Complete the following QC measures as one long series of commands using pipes:

* Rename columns to match style conventions #done
* Keep data from study weeks 1 or 3 #done
* Filter for individuals who were quantity compliant #none 
* Calculate the mean and median total SCFAs


we removed it earlier so now re-import
```{r}
scfa_wkly <- read_delim(file = "raw_data/SCFA_wkly.txt", 
                        delim = "\t", escape_double = FALSE, trim_ws = TRUE, na=c("NA"),
                        col_types = list())
...
```{r}
scfa_wkly_qc <- scfa_wkly %>%
  rename_all(tolower) %>% 
  filter(study_week == "week1" || study_week == "week3" ) %>% 
  filter(quantity_compliant == "none") %>% 
  mutate(median_scfa = acetate_median + butyrate_median + propionate_median, 
         mean_scfa = acetate_mean + butyrate_mean + propionate_mean
           ) 
...
write_delim(scfa_indv_qc, path = "curated_data/scfa_wkly_qc.txt", delim = "\t")
```
