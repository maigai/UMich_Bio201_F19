...
title: "Lab 5 new"
author: "Maira Gajda"
note: "Using curated data, am in tizzy"
date: "10/07/2019"
...
I would not determine the vectors to be equal. There is no overlap between the any of the quartiles of either box, or the largest and smallest outliers. 
...
These two vectors are roughly equal. There is significant visual overlap between the interquartile ranges of each vector, but they're skewed in opposite directions. 
...
```
# ••••• Are these two vectors equal? •••••
No there is no significant pverlap between the interquatile range of either vector, or any overla between the first and third quartiles. 
...
```
# ••••• Are these two vectors equal? •••••
There is some overlap between the means and the interquartile range of each vectors, but they still are shaped differently.
...
They're different concentrations. The 2x daily groups has a more consistent concentration of butyrate, throughout a smaller range. The 1x daily group has a large spike in the data and more variance throughout the range of data. 
...
# one times daily test group butyrate_long <- scfa_long %>%
...
# with a p value of .2367, I can say with 95% confidence that there is no meaningful difference between the group at week 1 and the group at week 3. 
...
```{r}
omit the median selet for the methane 



# first import the data frame, select forthe relevant  things, exclude the median. vajza a huaje ishte shum e emble, mos ki fr xhan
```
...
```{r}
# go back and remove the na not strictyl needed for this 
# e prisha mjuabetin 
scfa_wide %>%
...
```
lower n can still be used if its normal, but underpowered 
...
# dont forget to change the p values for the 
#some of theres are coming out the same
#look back at that in the future for now becasue thats a a bit funky 
s6 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS") 
shapiro.test(s6$delta_butyrate) #p-value = 0.2537
